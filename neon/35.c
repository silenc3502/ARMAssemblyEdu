...
int main(void)
{
	...
	int16_t low[4] = {1, 2, 3, 4};
	int16_t hi[4] = {5, 6, 7, 8};
	int16x4_t c_a, c_b, d_r;
	int16x8_t d_a, c_r;
	int i;

	c_a = vld1_s16(&low[0]);
	c_b = vld1_s16(&hi[0]);
	c_r = vcombine_s16(c_a, c_b);
	print_s16_vectors(c_r);

	for(i = 0; i < 4; i++)
		low[i] = hi[i] = 0;

	d_a = vld1q_s16(&in[0]);
	d_r = vget_high_s16(d_a);
	print_s16x4_vectors(d_r);

	d_r = vget_low_s16(d_a);
	print_s16x4_vectors(d_r);
